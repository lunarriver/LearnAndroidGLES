...
import lunarriver.learn.android.gles.a_primer.e_coordinate.MoreCubeActivity
...
class RotateBoxesActivity: BaseGLSurfaceActivity() {
    companion object {
        // 三角形顶点
    ...
    override fun onDrawFrame() {
        ...
        MoreCubeActivity.CUBE_POSITIONS.forEachIndexed { index, position ->
            // 模型矩阵
            val model = generateMat4fv()
            // 定位
            Matrix.translateM(model, 0, position[0], position[1], position[2])
            // 旋转
            Matrix.rotateM(model, 0, 20f * index + if (index % 3 == 0) rotateAngle else 0f , 1f, 0.3f, 0.5f)
            shaderProgram.setMat4("model", model)

            GLES20.glDrawArrays(
                GLES20.GL_TRIANGLES,
                0,
                MoreCubeActivity.VERTICES.size / (MoreCubeActivity.FLOATS_OF_VERTEX + MoreCubeActivity.FLOATS_OF_TEXTURE_COORDINATE)
            )
        }
